...
public class MechanicalZeroPosition extends RoboticsAPIApplication {
	...
	public void run() {
		...
		lbr.move(ptp(getApplicationData().getFrame("/Band")).setJointVelocityRel(.4) );
		lbr.move(ptp(getApplicationData().getFrame("/AndereSeite")).setJointVelocityRel(.4) );
		lbr.move(ptp(getApplicationData().getFrame("/ZurSeite")).setJointVelocityRel(.4) );
